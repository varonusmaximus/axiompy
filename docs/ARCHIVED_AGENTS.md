> **Archived (May 2026):** This file is a **frozen snapshot** of the former monolithic workspace rules. **Current normative guidance** lives in the **bundled Cursor skills** ([`bundles/axiompy_skills/`](../bundles/axiompy_skills/)), mirrored at [`.cursor/skills/`](../.cursor/skills/), plus short stubs at repo root ([`AGENTS.md`](../AGENTS.md), [`.cursorrules`](../.cursorrules)). Use skills as authority; treat everything below as **historical reference** only.
...
## Cursor skills vs this document (historical)

At the time this snapshot was taken, skills were thin playbooks that deferred here. **That split is obsolete:** skills and their **sidecar** markdown files now carry the normative checklists. The table below maps skill names to **which sections of this archive** corresponded to each topic (for archaeology only).

| Skill | Scope (use when) | Former detail in this archive |
|-------|------------------|-------------------------------|
| `code-review` | PR / change review, severity triage | Anti-patterns, smells, checklist, security — **Code Smells**, **Anti-Patterns**, **Validation**, **Testing** |
| `code-style` | Formatting, imports, line length, `match/case` | **Code Style & Formatting**, **Imports** |
| `design-patterns` | Factories, settings, DI, HTTP client choice | **Design Patterns**, **HTTPClient Pattern**, **Validation** |
| `documentation` | Docstrings, READMEs, diagrams in docs | **Documentation** (module docstrings, README standards) |
| `testing` | pytest layout, coverage, mocks | **Testing** |

Workspace **`.cursorrules`** was a short mirror for Cursor in-repo; it was not installed by `axiompy-skills`. See root **README** (Cursor skills) for how guidance layers fit together today.
